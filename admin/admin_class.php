...
function verifyMobileNumber($mobileNumber) {
    ...
    if (preg_match('/^94\d{9}$/', $digitsOnly)) {
...
function login2()
{
	...
	function add_to_cart(){
		...
		$data = " product_id = $pid ";	
		$qty = isset($qty) ? $qty : 1 ;
		$prod_owner = isset($prod_owner) ? $prod_owner : 0; // Corrected
		$data .= ", qty = $qty ";	
		$data .= ", prod_owner = $prod_owner ";
	...
	function save_culture_order()
	{
		if(isset($_SESSION['login_user_id']))
		{
			...
			$type = 0; // Assuming type value is 0
			$data .= ", type = " . $type;
			...
			if($save){
				...
				$qry = $this->db->query("SELECT * FROM cart where user_id =".$_SESSION['login_user_id']);
				//$qry = $this->db->query("SELECT cart.*, product_list.price FROM cart INNER JOIN product_list ON cart.product_id = product_list.id WHERE cart.user_id =" . $_SESSION['login_user_id']);
				while($row= $qry->fetch_assoc()){

						$data = " order_id = '$order_id' ";
						$data .= ", product_id = '".$row['product_id']."' ";
						$data .= ", qty = '".$row['qty']."' ";
						$data .= ", prod_owner = '".$row['prod_owner']."' ";

						/*$data2 = " user_id = '".$row['prod_owner']."' ";
						$type = 1; // Assuming type value is 1
						$data2 .= ", type = " . $type;
						$data2 .= ", totalAmount = " . $row['price'];
						$save3 = $this->db->query("INSERT INTO orders set ".$data2);
						if($save3)
						{
							$id_earning = $this->db->insert_id;
							$data3 = " order_id = '$id_earning' ";
							$data3 .= ", product_id = '".$row['product_id']."' ";
							$data3 .= ", qty = '".$row['qty']."' ";
							$data3 .= ", prod_owner = '".$row['prod_owner']."' ";
							$save2=$this->db->query("INSERT INTO order_list set ".$data3);
						}*/
