...
<script>
	...
	/*$('#checkout').click(function(){
		if('<?php echo isset($_SESSION['login_user_id']) ?>' == 1){
			...
			//location.replace("index.php?page=confirmOrder&total=<?php echo $total; ?>")
			
			$.ajax({
                url:"admin/ajax.php?action=save_culture_order",
                method:'POST',
                //data:$(this).serialize(),
				data: $(this).serialize() + "&totalAmount=" + <?php echo $total; ?>,
                success:function(resp){
                    if(resp==1){
                        swal({
	...
	/*$('#checkout').click(function() {
		if ('<?php echo isset($_SESSION['login_user_id']) ?>' == 1) {
			// Retrieve the total amount from the input field
			
			var totalAmount = parseFloat($('input[name="TotPrice"]').val());
			// Redirect to the server-side script that initiates the Stripe Checkout session
			window.location.href = 'your_server_script.php?total=' + totalAmount;
		} else {
			uni_modal("Checkout", "login.php?page=checkout");
		}
	});*/
    ...
    if ('<?php echo isset($_SESSION['login_user_id']) ?>' == 1) {
        ...
        $.ajax({
            url: "admin/ajax.php?action=save_culture_order",
            method: 'POST',
            data: $(this).serialize() + "&totalAmount=" + totalAmount,
            success: function(resp) {
                if (resp == 1) {
                    // Success alert
                    swal({
